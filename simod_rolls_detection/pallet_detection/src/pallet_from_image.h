...
class PalletFromImage
{
  ...
  cv::Mat CloseImage(const cv::Mat & image, const size_t closing_size);
  cv::Mat DilateImage(const cv::Mat & image, const size_t dilate_size);
  template <typename T>
  cv::Mat SimpleEdgeImage(const cv::Mat & image);

  // int32 image
  // float depth_image
  cv::Mat PlaneEdgeImage(const cv::Mat & image, const cv::Mat & depth_image, const Vector4fVector & found_planes, const int32 relevant_plane);
